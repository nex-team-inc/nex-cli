...
def get_keystore_path(name, check=True):
    keystore_path = os.path.realpath(os.path.join(os.path.dirname(__file__), 'secrets', name + '.jks'))
...
def get_keystore_password_path(name, check=True):
    ...
    if check and not os.path.isfile(keystore_password_path):
        raise click.ClickException(f"Keystore password file not found: {keystore_password_path}")
...
def get_keystore_password(name):
    ...
    key_name = client.crypto_key_path(GCP_PROJECT, GCP_KMS_LOCATION, GCP_KMS_KEYRING, name)
...
def print_cert(signer):
    ...
    try:
        ...
        raise click.ClickException(f"Failed to list keystore entries: {e.stderr.decode('utf-8')}")
...
def export_cert(signer, output):
    ...
    try:
        ...
        raise click.ClickException(f"Failed to export signing certificate: {e.stderr.decode('utf-8')}")
...
def create_cert(signer, dname):
    ...
    keystore_path, keystore_password_path = get_keystore_password_path(signer, check=False)
    ...
    client = kms.KeyManagementServiceClient()
    key_name = client.crypto_key_path(GCP_PROJECT, GCP_KMS_LOCATION, GCP_KMS_KEYRING, signer)
    ...
    if crypto_key is None:
        ...
        click.echo(f"... using existing GCP KMS key created at {crypto_key.create_time}")

    click.echo(f"... generating and encrypting new keystore password")
    keystore_password = generate_password()
    encrypt_response = client.encrypt({'name': key_name, 'plaintext': keystore_password.encode('utf-8')})
    ...
    try:
        ...
        raise click.ClickException(f"Failed to create new signing certificate: {e.stderr.decode('utf-8')}")
